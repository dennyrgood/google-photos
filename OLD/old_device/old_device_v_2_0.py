...
class AssistantUI:
    ...
    def __init__(self, root):
        ...
        self.launch_default_btn = ttk.Button(launch_frame, text='LAUNCH BROWSER', command=lambda: self.launch_with_mode('default'))
        ...
        self.keyboard_status = ttk.Label(main, text='Keyboard: Click window to focus, then press keys', font=('Courier', 9), foreground='blue')
        ...
        def _backspace():
            try:
                if not self.browser._running:
                    return
                self.browser._cmd_queue.put(('backspace', None))
            except Exception as e:
                print(f'[BACKSPACE] ERROR: {e}')
        threading.Thread(target=_backspace, daemon=True).start()
    ...
    def _on_browser_ready(self):
        ...
        for b in getattr(self, 'name_buttons', []):
            try:
                ...
                pass
        self.keyboard_status.config(text='Keyboard: READY - Press N/P for next/prev, or name keys', foreground='green')
    def next_photo(self):
